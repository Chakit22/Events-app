...
The app lets a mocked user view events, RSVP as attending or not attending, filter events by RSVP status, and lets an admin create, edit, and delete events. The backend provides a minimal in-memory events API for learning frontend-backend data flow.
...
The backend currently stores events in memory. Data resets when the server restarts.
...
Production concerns are intentionally omitted for now, including database persistence, protected routes, deployment configuration, advanced API validation, authorization checks, and full frontend-backend mutation syncing.
